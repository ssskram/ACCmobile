...
export default class AllIncidents extends React.Component<props, state> {
  constructor(props) {
    ...
    this.state = {
      onFilter: false,
      format: "cards",
      incidents: props.incidents.sort(
        (a, b) => +new Date(b.date) - +new Date(a.date)
      ),
      originalIncidents: props.incidents.sort(
        (a, b) => +new Date(b.date) - +new Date(a.date)
      ),
      currentPage: 1,
      incidentsPerPage: 10,
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props != nextProps) {
      if (this.state.onFilter === false) {
        this.setState({
          incidents: nextProps.incidents.sort(
            (a, b) => +new Date(b.date) - +new Date(a.date)
          ),
          originalIncidents: nextProps.incidents.sort(
            (a, b) => +new Date(b.date) - +new Date(a.date)
          ),
