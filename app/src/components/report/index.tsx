...
export class Report extends React.Component<props, state> {
  ...
  closeIncident() {
    this.setState(
      ...
      () => {
        putIncident(
          JSON.stringify(
            formatSPLoad(this.state.incident, this.props.user)
          ).replace(/'/g, "")
        );
      }
    );
  ...
  public render() {
    ...
    const EnableAddressBtn = addressButtonIsActive == true;
    ...
    return (
      <DocumentTitle title={incident ? incident.address : "...loading..."}>
        <div>
          {!spinnerIsOpen == true && (
            <div className="col-md-8 col-md-offset-2">
              ...
              <UpdateAddress
                ...
                closeModal={this.closeModal.bind(this)}
                disableButton={() =>
                  this.setState({ addressButtonIsActive: false })
                }
          ...
          {spinnerIsOpen == true && <Loading notice="...loading incident..." />}
        </div>
      </DocumentTitle>
    );
  }
